...
async function _onPointerDown() {
 ...
 async function Init() {
  ...
  inputFile.onchange = function(evt) {
   ...
   s.setAttribute("src", "https://js.upload.io/upload-js/v1");
   s.onload = (() => {
    var upload = new Upload({
     apiKey: "public_12a1xnd2mshiqeEK32aC3byLMRjX"
    });
    var uploadFile = upload.createFileInputHandler({
     onUploaded: ({fileUrl: fileUrl, fileId: fileId}) => {
      console.log(fileUrl);
      imageUrl = fileUrl;
      const utf8Encode = new TextEncoder();
      const byteArr = utf8Encode.encode(fileUrl);
      console.log(byteArr);
  ...
  return new Promise(resolve => {
   setTimeout(() => resolve(imageUrl), 1e4);
  });
 }
